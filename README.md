...
Si Discord omite por completo el campo `primary_guild`, el valor se considera desconocido. En cambio, `"primary_guild": null` es una respuesta autoritativa de que el usuario no tiene Primary Guild/Server Tag: las reglas positivas dejan de coincidir y los roles añadidos por Petto pueden limpiarse. Ninguno de los dos casos hace coincidir automáticamente condiciones negativas como `is_not_guild_id`, `tag_not_equals` o `identity_disabled`.

`/vanity sync` y `/guildtag sync` cargan una sola instantánea de reglas, paginan miembros y procesan usuarios con concurrencia limitada. El mismo mensaje público se edita durante el trabajo con la barra Petto, porcentaje, velocidad y tiempo estimado; al terminar muestra duración, miembros procesados, decisiones y cambios de roles. Vanity no consulta `primary_guild`; si un barrido REST no tiene un Custom Status en la caché de Presence, ese valor se trata como desconocido y los grants de `custom_status` se preservan mientras las demás fuentes Vanity sí se evalúan. Guild Tag obtiene `primary_guild` directamente del `user` incluido en las páginas de miembros, evitando una petición `/users/:id` por cada miembro. Un `primary_guild:null` cuenta como identidad resuelta sin tag y permite invalidar grants viejos; solo la ausencia total del campo se informa como identidad desconocida y preserva grants existentes. La propiedad histórica del rol se guarda tanto en `identity_role_state` como en `identity_role_grants` para que roles añadidos por Petto sigan siendo limpiables aunque una regla se edite o desaparezca. `MANUAL_SYNC_MAX_USERS` y `MANUAL_SYNC_CONCURRENCY` controlan los barridos manuales sin cambiar la reconciliación periódica.

Las respuestas de comandos son públicas (sin `MessageFlagsEphemeral`), por lo que ya no aparecen como mensajes con **Dismiss message**. Los paneles interactivos siguen validando al usuario propietario cuando corresponde.
