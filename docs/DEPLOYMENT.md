...
Role changes for the same member are serialized through fixed striped identity locks while different members may be evaluated concurrently. `discordgo`'s REST limiter/retries remain enabled, so manual worker concurrency does not bypass Discord rate limits. The engine writes a grant intent before the Discord mutation and reconciles the next evaluation if Discord fails after the database transaction.

## Server Tag limitation

Discord's `primary_guild` object contains `identity_guild_id`, nullable `identity_enabled`, `tag`, and `badge`; `tag` is limited to four characters. The current `discordgo` typed User model does not include this newer object, so raw `USER_UPDATE` decoding is used. Discord does not guarantee that every Server Tag change arrives as a guild-member event. An omitted `primary_guild` field is treated as unknown, while an explicit `primary_guild: null` is treated as authoritative no-tag data. Negative comparisons still do not match absent values. The bot therefore avoids unbounded restart-time member polling, processes available events, and exposes `/identity sync`, `/vanity sync`, and `/guildtag sync` as bounded manual recovery commands. Member collection is paginated in chunks of at most 1,000 and hard-capped at 10,000 per guild. Periodic reconciliation uses `RECONCILIATION_MAX_USERS`; explicit Vanity/Guild Tag sync uses `MANUAL_SYNC_MAX_USERS` (default 1,000) and `MANUAL_SYNC_CONCURRENCY` (default 4). Guild Tag manual sync reads `primary_guild` directly from the paginated member payload instead of issuing one user lookup per member. Explicit null values are resolved as no tag; only omitted fields preserve prior grants as unknown. Periodic reconciliation stays off unless the environment explicitly enables it.
