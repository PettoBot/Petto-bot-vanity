...
`/vanity sync` y `/guildtag sync` cargan una sola instantánea de reglas, paginan miembros y procesan usuarios con concurrencia limitada. El mismo mensaje público se edita durante el trabajo con la barra Petto, porcentaje, velocidad y tiempo estimado; al terminar muestra duración, miembros procesados, decisiones y cambios de roles. Vanity no consulta `primary_guild`; si un barrido REST no tiene un Custom Status en la caché de Presence, ese valor se trata como desconocido y los grants de `custom_status` se preservan mientras las demás fuentes Vanity sí se evalúan. Guild Tag obtiene `primary_guild` directamente del `user` incluido en las páginas de miembros, evitando una petición `/users/:id` por cada miembro. Si Discord omite ese campo, se informa como cobertura de identidad no disponible, no como warning, y los grants existentes se preservan. `MANUAL_SYNC_MAX_USERS` y `MANUAL_SYNC_CONCURRENCY` controlan los barridos manuales sin cambiar la reconciliación periódica.
